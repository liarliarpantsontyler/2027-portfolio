...
for (const name of process.argv.slice(2).length
  ...
  if (clocks.includes(name)) {
    ...
    await ffmpeg([
      ...
      input,
      "-filter_complex",
      "[0:v]split=2[a][b];[a]trim=start=1:end=15,setpts=PTS-STARTPTS[body];[b]trim=start=0:end=1,setpts=PTS-STARTPTS[head];[body][head]xfade=transition=custom:duration=1:offset=13:expr='A*(P*P*(3-2*P))+B*(1-P*P*(3-2*P))',format=yuv420p[v]",
      "-map",
      "[v]",
      ...encoding,
    ]);
    ...
    const duration =
      name === "onboarding" ? 30 : name === "customization" ? 32 : 14;
    const color =
      name === "hero" || name === "onboarding" ? "0xf6f5f1" : "0x101012";
    await ffmpeg([
      "-i",
      input,
      "-vf",
      `fade=t=in:st=0:d=0.25:color=${color},fade=t=out:st=${duration - 0.3}:d=0.3:color=${color}`,
      ...encoding,
    ]);
  }
  ...
  const posterTime =
    name === "customization" ? 8 : name === "fullscreen" ? 7 : 1;
  await ffmpeg([
    "-ss",
    String(posterTime),
    "-i",
